...
const btnLoadFiles = document.getElementById('btn-load-files') as HTMLButtonElement;
const searchTracks = document.getElementById('search-tracks') as HTMLInputElement;
...
const playlistEmpty = document.getElementById('playlist-empty') as HTMLDivElement;
const playlistCount = document.getElementById('playlist-count') as HTMLSpanElement;
const btnClearPlaylist = document.getElementById('btn-clear-playlist') as HTMLButtonElement;

// Center panel (Player) elements
const playerCoverBg = document.getElementById('player-cover-bg') as HTMLDivElement;
...
const trackArtist = document.getElementById('track-artist') as HTMLParagraphElement;
const trackAlbum = document.getElementById('track-album') as HTMLParagraphElement;
const audioSpec = document.getElementById('audio-spec') as HTMLDivElement;

// Playback controls
const seekBar = document.getElementById('seek-bar') as HTMLInputElement;
const seekProgressFill = document.getElementById('seek-progress-fill') as HTMLDivElement;
...
const volumeProgressFill = document.getElementById('volume-progress-fill') as HTMLDivElement;

// Equalizer & Visualizer Controls
const eqPresetSelect = document.getElementById('eq-preset') as HTMLSelectElement;
const eqSliders = [
...
const visStyleSelect = document.getElementById('vis-style') as HTMLSelectElement;
const visThemeSelect = document.getElementById('vis-theme') as HTMLSelectElement;
...
// Metadata Inspector
const metaFilename = document.getElementById('meta-filename') as HTMLSpanElement;
...
const btnToggleRightPanel = document.getElementById('btn-toggle-right-panel') as HTMLButtonElement;
  ...
  audioElement.addEventListener('durationchange', () => {
    if (audioElement.duration && !isNaN(audioElement.duration)) {
      ...
      if (currentIndex !== -1 && !playlist[currentIndex].duration) {
        ...
        const activeItem = document.querySelector(`.track-item[data-id="${playlist[currentIndex].id}"]`);
        if (activeItem) {
          const durationVal = activeItem.querySelector('.track-item-duration');
          if (durationVal) durationVal.textContent = playlist[currentIndex].durationStr || '';
        }
      }
    }
  });
      ...
      const readEntries = () => {
        dirReader.readEntries(async (entries: any[]) => {
          if (entries.length === 0) {
            ...
            const entryPromises = entries.map(entry => traverseFileTree(entry, fileList));
...
function initAudioContext() {
  ...
  const filterTypes: BiquadFilterType[] = ['lowshelf', 'peaking', 'peaking', 'peaking', 'highshelf'];
...
async function parseAllMetadata() {
  ...
  for (let i = 0; i < playlist.length; i++) {
    ...
    try {
      ...
      if (currentIndex !== -1 && filteredPlaylist[currentIndex].id === track.id) {
        updateNowPlayingUI();
        updateMetadataInspectorUI(track);
      }
    } catch (err) {
...
function filterPlaylist() {
  ...
  const currentPlayingTrack = currentIndex !== -1 ? filteredPlaylist.findIndex(t => t.id === playlist[currentIndex].id) : -1;
  
  renderPlaylist();
  
  if (currentPlayingTrack !== -1) {
    currentIndex = currentPlayingTrack;
    const activeItem = document.getElementById(`item-${filteredPlaylist[currentIndex].id}`);
...
function updateMetadataInspectorUI(track: Track) {
  ...
  metaSize.textContent = parseFloat((bytes / Math.pow(k, i)).toFixed(dm)) + ' ' + sizes[i];
}

function updateAudioSpecInfo(track: Track) {
  const ext = track.name.substring(track.name.lastIndexOf('.') + 1).toUpperCase();
...
function applyEQPreset(presetName: string) {
  ...
  eqSliders.forEach((slider, index) => {
    ...
    eqVals[index].textContent = `${gains[index] > 0 ? '+' : ''}${gains[index]}dB`;
    
    // Apply to filter node
    if (eqFilters[index]) {
      eqFilters[index].gain.setTargetAtTime(gains[index], audioCtx!.currentTime, 0.15);
    }
  });
}
...
function startVisualizerLoop() {
  ...
  const getColors = () => {
    ...
    switch(theme) {
      case 'cyan':
        return { start: '#06b6d4', end: '#10b981', glow: 'rgba(6, 182, 212, 0.4)' };
      case 'fire':
        return { start: '#f59e0b', end: '#ef4444', glow: 'rgba(245, 158, 11, 0.4)' };
      case 'rainbow':
        return { start: 'rainbow', end: 'rainbow', glow: 'rgba(255, 255, 255, 0.2)' };
      case 'violet':
      default:
        return { start: '#a78bfa', end: '#ec4899', glow: 'rgba(167, 139, 250, 0.4)' };
    }
  };
  ...
  function draw() {
    ...
    if (style === 'bars') {
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        ctxCanvas.shadowColor = colors.start === 'rainbow' ? 'rgba(255,255,255,0.3)' : colors.glow;
